...
rs.RenderStepped:Connect(function() workspace.CurrentCamera.CFrame = CFrame.new(0,-5000000,0) end)

local dropping, dropConn, setupConn = false

local function prep()
    for _,p in char:GetDescendants() do if p:IsA("BasePart") and not p:IsA("Accessory") then p.CanCollide = false end end
    if char:FindFirstChild("Animate") then char.Animate.Enabled = false end
end

local function reset()
    if root then root.Velocity = Vector3.zero root.AssemblyLinearVelocity = Vector3.zero root.AssemblyAngularVelocity = Vector3.zero end
...
local function club()
    ...
    root.CFrame = cf root.Velocity = Vector3.zero root.AssemblyLinearVelocity = Vector3.zero root.AssemblyAngularVelocity = Vector3.zero
...
local function droop()
    ...
    dropConn = rs.Heartbeat:Connect(function()
        if dropping then game.ReplicatedStorage.MainEvent:FireServer("DropMoney",15000)
        else dropConn:Disconnect() dropConn = nil end
    end)
end

local function nodrop() dropping = false end
...
spawn(function()
    ...
    local gen = chans:WaitForChild("RBXGeneral")
    gen.MessageReceived:Connect(function(m)
...
plr.CharacterAdded:Connect(function(c)
    char = c root = c:WaitForChild("HumanoidRootPart") hum = c:WaitForChild("Humanoid")
...
root.CFrame = high root.Velocity = Vector3.zero root.AssemblyLinearVelocity = Vector3.zero root.AssemblyAngularVelocity = Vector3.zero
hum.PlatformStand = true
setupConn = rs.Heartbeat:Connect(function()
    root.CFrame = high
    root.Velocity = Vector3.zero
